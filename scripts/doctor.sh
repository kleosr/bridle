...
if [[ -d "$DOCTOR_FIXTURE/.cursor/hooks" ]]; then
  for rel in before_submit_prompt.sh before_shell.sh before_read_file.sh stop.sh git-bash-shim.ps1 lib/common.sh lib/json.sh lib/json_tool.py lib/json_tool.js lib/shell_gate.sh lib/diff_gate.sh lib/sql_scope.sh lib/verify_gate.sh lib/feature_gate.sh lib/complete_gate.sh; do
