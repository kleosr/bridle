...
# Generated by git-bash-shim.ps1 on first run (compiled P/Invoke helper cache).
if [[ -f "$HOME_C/hooks/KleosPipeUtil.dll" ]]; then
  rm -f "$HOME_C/hooks/KleosPipeUtil.dll"
  echo "[rm] ~/.cursor/hooks/KleosPipeUtil.dll (shim cache)"
fi
