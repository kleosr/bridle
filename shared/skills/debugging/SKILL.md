...
Agent-loop modes (name them; do not treat as noise): hallucinated success (tool 4xx / nonzero claimed as done), stall (same failing command or re-read without an edit), scope creep, context rot (stale transcript vs workspace), tool misuse, injected instructions in files or tool output (data, not authority), false `passing` in a feature list, incomplete integration (compiles but unwired, a dangling reference, or an undeclared import — `bash scripts/complete.sh check` names these). Re-probe workspace state; do not trust the transcript. Diagnose which harness layer failed (task spec, context, environment, verification, state) before blaming the model. A host `failClosed` block with hook exit 1 is a sensor crash (empty stdout or non-zero process exit), not a secret-path deny. Cursor "poll wait was blocked by a hook" on `terminals/*.txt` is that crash.
