...
run_test "regression: read allows Cursor Read path field" "allow" "$(jq -n --arg p /repo/README.md '{path:$p}' | bash "$PACK/shared/hooks/before_read_file.sh" | jq -r '.permission // "none"')"
...
BIG_PERM="$(jq -n --arg p 'C:\Users\u\.cursor\projects\repo\terminals\1.txt' --rawfile c "$BIG_BLOB" '{path:$p, contents:$c}' | bash "$PACK/shared/hooks/before_read_file.sh" | jq -r '.permission // "none"')"
rm -f "$BIG_BLOB"
