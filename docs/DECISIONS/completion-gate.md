...
- `shared/hooks/lib/complete_gate.sh` — deterministic detectors over the working tree vs `HEAD`:
  ...
  - **orphan**: a source module *added* by the change whose basename is never referenced by any other file — added but not wired/registered/imported. Entrypoints (`index`, `main`, `__init__`…), framework auto-discovery dirs (`app/`, `pages/`, `routes/`, `migrations/`…), and test files are excluded so the detector stays high-precision.
  - **todo**: ownerless `TODO/FIXME/XXX/HACK` added in the change (a light nudge, not a block).
- `scripts/complete.sh check [dir]` — aggregates those signals plus changed-file syntax (reusing `verify_gate.sh`) into a deterministic **confidence 0-100** and a **verdict**: `act` (exit 0) when confidence ≥ `COMPLETE_MIN_CONFIDENCE` (default 80), else `escalate` (exit 3). Conflicts and syntax errors are *critical* → confidence 0. A caller loop or CI step can branch on the exit code.
