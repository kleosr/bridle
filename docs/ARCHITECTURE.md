...
Hook stdout is JSON. Scripts treat missing input, malformed payloads, missing policy, or an unavailable Python/Node JSON codec as failure. The host's handling of `failClosed`, `ask`, Read denies, and cloud lanes is recorded in `docs/host-capability.md`; it is not guaranteed by script tests.
